...
| Тесты | `.venv/bin/python -m pytest`, 2528 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2528 тестов (прогон 2026-09-12)
```
...
XML-обработчиков вместо ложного статического `passed`. Успешная статическая операция не считается
проверкой Registry, нативным импортом или визуальной приёмкой: эти уровни
остаются `not_checked`.
