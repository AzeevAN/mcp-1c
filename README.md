...
| Тесты | `.venv/bin/python -m pytest`, 1697 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1697 тестов (прогон 2026-08-31)
```
...
вопроса параметр опускается. `include_explicit` добавляет служебные и словарные
карточки внутри обычного раздела; полностью специальные разделы открываются
уже своим явным `domain`. `include_hidden` применяется только по прямой просьбе
об архивном поведении. Методы, свойства и объекты платформы не относятся к
общей справке и по-прежнему ищутся через `search_syntax` / `get_syntax`.
