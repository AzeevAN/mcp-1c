...
| Тесты | `.venv/bin/python -m pytest`, 1940 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1940 тестов (прогон 2026-09-03)
```
...
поиск конкретного объекта без загрузки роли целиком. Обычный режим группирует
предоставленные права карточками и не рисует строки `false`; дочерние цели и
технический аудит открываются вкладками одного модального окна. Каталог,
объекты, детали и кандидаты читаются ограниченными серверными страницами, RLS
раскрывается только отдельной кнопкой. Состояния пустого Registry, `missing` и
`error` объясняются, а операций назначения или редактирования ролей на
странице нет.
