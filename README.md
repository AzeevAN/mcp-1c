...
необязательная общая справка подключается готовым read-only файлом SQLite.
...
| Общая справка | опциональный адаптер schema v1; без доверенного файла две дополнительные ручки не регистрируются |
| Дашборд | рабочая SPA и классический совместимый UI |
| Авторизация | `API_TOKEN` на чтение, `ADMIN_TOKEN` на запись |
| Тесты | `.venv/bin/python -m pytest`, 1731 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1731 тест (прогон 2026-08-30)
```
...
| `MCP1C_REFERENCE_DB` | `off`, пусто либо путь к SQLite внутри контейнера | пусто: управляемый `data/reference/reference.sqlite3` |
| `MCP1C_REFERENCE_TRUST_UNSIGNED` | временно разрешить неподписанную schema v1 | пусто: неподписанный файл отвергается |
...
`MCP1C_REFERENCE_TRUST_UNSIGNED=1` существует только для локальной приёмки
экспериментальной ветки. Это не режим эксплуатации: слияние и деплой должны
оставаться заблокированы, пока артефакт не подписывается отдельно и сервер не
проверяет подпись доверенным публичным ключом.
...
проектом. При управляемом пути администратор выбирает файл `.sqlite3` в общей
форме «Загрузить источник» на странице «Источники» в classic или SPA. Та же
форма принимает ZIP/HBK/JSON, а по расширению направляет SQLite в отдельный
справочный адаптер, не смешивая её с Registry. Сервер проверяет размер, заголовок
SQLite, точную схему, целостность, внешние ключи, JSON-поля, логический SHA-256
и доверие к подписи. Некорректный файл не заменяет прежний.

Успешно проверенный файл сохраняется как
`data/reference/reference.sqlite3`, но текущий процесс продолжает обслуживать
...
`reference.sqlite3`; точное имя можно скопировать кнопкой в диалоге. Файл и
расходный индекс снимаются сразу. Если две ручки
...
Внешний `MCP1C_REFERENCE_DB=/data/другой-файл.sqlite3` оставляет файл под
...
| Общая SQLite | отдельный лимит 32 МиБ, read-only open, строгая schema v1, integrity/FK/JSON/hash и подпись до активации |
...
Экспериментальный обход подписи предназначен только для локальной разработки.
До появления проверенного формата подписи этот контур нельзя сливать в релизную
ветку или развёртывать как рабочий сервер.
