...
Сервер принимает `shquery_ru.hbk` отдельным видом и отвечает по нему наравне
со справкой платформы. Подсказка на `/sources` и списки — дашбордный и
`reg-list` — обязаны говорить о нём правду: прежде подсказка прямо утверждала,
что файл не подходит, а загруженный источник в
...
def test_подпись_переключателя_syntax_называет_язык_запросов(tmp_path):
    ...
    языку запросов (`shquery_ru.hbk`), который входит в тот же индекс, что и
    справка платформы."""
    client, _ = client_for(tmp_path)
