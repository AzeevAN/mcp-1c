...
| Тесты | `.venv/bin/python -m pytest`, 1933 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1933 теста (прогон 2026-09-03)
```
...
Архивы, уже находящиеся в `data/incoming/`, и настроенный read-only источник
не имеют лимита сырого файла: это путь для больших выгрузок без HTTP-копии.
Для всего входа сервер всё равно проверяет безопасные и однозначные пути,
symlink, специальные файлы, шифрование и число записей. Лимит одного элемента,
коэффициент сжатия и суммарный распакованный бюджет применяются только к
элементам, которые probe или parser действительно открывает; известный
игнорируемый балласт не распаковывается и в этот бюджет не входит.
...
не содержит текст RLS: она возвращает размер, `restriction_ref` и
упорядоченный массив `fields`. Конкретное условие или шаблон читается тем же
инструментом по явной ссылке и `restriction_cursor` окнами до 8 000 символов
без необратимого обрезания. Пустой `condition` остаётся отдельным условным RLS
с теми же `fields`.
