...
def path_for(cache_dir: str | Path, source_id: str, kind: str) -> Path:
    ...
    Идентификатор источника — имя конфигурации из манифеста, там встречается
    и косая черта, и двоеточие; в имя файла они не годятся.
    """
    safe = "".join(ch if ch.isalnum() or ch in "-_. " else "_" for ch in source_id)
    return Path(cache_dir) / f"{safe}.{kind}"
