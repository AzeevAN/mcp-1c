...
| Тесты | `.venv/bin/python -m pytest`, 2684 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2684 теста (прогон 2026-09-13)
```
...
Панель команд `command_bar` требует одну или более прямых кнопок и всегда
получает `ExtendedTooltip`; доступны необязательные `title`,
`horizontal_location`, `horizontal_stretch` и `vertical_stretch`. Собственных
событий у панели нет. `Popup`, `ButtonGroup`, автоматический `CommandSource` и
панели без явных кнопок пока сохраняются только в inventory.
...
18 158 байт и приблизительно **4 257 токенов** для четырёх схем Forms при
