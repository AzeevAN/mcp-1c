...
| Тесты | `.venv/bin/python -m pytest`, 2746 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2746 тестов (прогон 2026-09-13)
```
...
вида `Обработка.ИмяОбъекта`; compiler преобразует её в точный `cfg:*Object`
тип, decompiler восстанавливает обратно, а поле может ссылаться на вложенный
путь вида `Объект.Поле`. Одиночные ссылочные значения задаются как
...
21 032 байта и приблизительно **4 988 токенов** для четырёх схем Forms при
