...
class Registry:
    ...
    def _extract_code(
        ...
        у манифеста — воспроизведено ре-ревью; то же дали бы кончившееся
        место или права) и пустой результат (все члены отсеклись
        санитизацией `safe_target`) прежний разбор не трогают вовсе — до
        какой-либо записи в `корень` дело не доходит. Раньше `корень`
        чистился ДО распаковки, и это было неверно само по себе: реестр
        продолжал висеть со `status=ready` и старым `items_total`, а на
        диске лежал пустой или полупустой каталог.
        ...
        переданный `drop`. Второе переименование тоже под защитой: не
        удалось — отставленное возвращается на место корня, временный
        каталог убирается, и наверх летит то же исключение. Это не
        ...
        rename с `ENOTEMPTY` и наполовину снёс прежний разбор. Отставленный
        каталог для такого исхода безопасен: даже недочищенный, он больше не
        корень.

        Что НЕ гарантируется: рокировка — два отдельных системных вызова
        (`rename`, `rename`), а не одна атомарная операция. Окно между ними
        — доли секунды, а не три секунды распаковки или секунда `rmtree` на
        тысячах файлов, но не ноль: убийство процесса ровно между двумя
        `rename` оставило бы `корень` не существующим вовсе (отставленный
        уже создан, временный ещё не переехал). Try/except такое не ловит —
        это ограничение модели «переименование, а не журналируемая
        транзакция», а не забытый случай.
        """
        from . import intake
        ...
        if корень.exists():
            ...
            # она уже гарантированно уникальна, второй `mkdtemp` ради того
            # же не нужен: `отставленный` только резервирует имя под
            # `rename`, создавать его заранее незачем.
            ...
            try:
                ...
                # Второй rename не удался ДО переноса — временный каталог с
                # уже распакованным содержимым остался лежать сам по себе,
                # никто на него больше не ссылается. Не убрать — значит
                # оставить мусор рядом с каждым отказом переразбора.
                shutil.rmtree(временный, ignore_errors=True)
                raise
            drop(отставленный)
        else:
