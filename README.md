...
| Тесты | `.venv/bin/python -m pytest`, 1909 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1909 тестов (прогон 2026-09-02)
```
...
`condition` сохраняется как условное право, а не исчезает. Повреждение или
