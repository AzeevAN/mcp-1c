...
| Тесты | `.venv/bin/python -m pytest`, 2415 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2415 тестов (прогон 2026-09-12)
```
...
capability и не появляется в `tools/list`. Его текущий внутренний compiler
принимает строгое подмножество спецификации управляемой формы, детерминированно
возвращает тексты `Form.xml` версии `2.16` и `Form/Module.bsl` с UTF-8 BOM и
CRLF, но не пишет их на диск и не обращается к Registry или 1С. Успешная
статическая сборка не считается нативным импортом или визуальной приёмкой:
эти уровни результата остаются `not_checked`.
