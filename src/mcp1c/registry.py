...
class Registry:
    ...
    def _extract_code(
        ...
        замена на его место только после успеха.

        Раньше `корень` чистился ДО распаковки: любая ошибка `intake.extract`
        (битый CRC у модуля, кончившееся место, права) или пустой результат
        (все отобранные члены отсеклись санитизацией `safe_target`) оставляли
        `корень` пустым или полупустым, а источник в реестре продолжал висеть
        со `status=ready` и старым `items_total` — реестр и диск расходились.
        Ре-ревью воспроизвело это на битом CRC у модуля (не у манифеста):
        распознавание прошло, корень снесён, `extract` упал, старого кода
        нет, реестр врёт.

        Теперь распаковка идёт в СОСЕДНИЙ временный каталог того же
        родителя — та же файловая система, значит финальная замена
        (`rename`) атомарна, а не копирование — и старый `корень` сносится
        (через переданный `drop`) только когда `intake.extract` уже
        гарантированно отработала и вернула непустой результат. Любое
        исключение или пустой результат оставляют прежний разбор нетронутым:
        временный каталог убирается, старый остаётся как был.
        """
        from . import intake
        ...
        try:
            ...
            if not файлов:
                ...
                # `extract` — до `drop(корень)` ниже дело не доходит.
