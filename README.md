...
| Тесты | `.venv/bin/python -m pytest`, 2298 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2298 тестов (прогон 2026-09-12)
```
...
| `get_object` | поля, их доказанное происхождение, HTTP-endpoint, XDTO-пакеты и типы, таблицы запроса, связи и кодовые сведения объекта |
