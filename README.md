...
| Тесты | `.venv/bin/python -m pytest`, 2530 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2530 тестов (прогон 2026-09-12)
```
...
Замер 2026-09-12 по каноническому JSON фактической дельты `tools/list` дал
11 506 байт и приблизительно **2 747 токенов** для четырёх схем Forms при
