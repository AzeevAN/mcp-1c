...
| Тесты | `.venv/bin/python -m pytest`, 2251 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2251 тест (прогон 2026-09-12)
```
...
график. Серый стандартный реквизит Конфигуратора, который нельзя выбрать в
запросе, не выдаётся как доступное поле.
