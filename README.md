...
| Тесты | `.venv/bin/python -m pytest`, 2230 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2230 тестов (прогон 2026-09-10)
```
...
- получает готовый образ `ghcr.io/azeevan/mcp-1c:3.0.0` без локальной сборки;
...
1. Обычный пользователь скачивает готовый
   `ghcr.io/azeevan/mcp-1c:3.0.0` и запускает один `compose.yaml` по инструкции
   ниже.
...
curl --fail --show-error --location --output compose.yaml \
  https://raw.githubusercontent.com/AzeevAN/mcp-1c/v3.0.0/compose.yaml
curl --fail --show-error --location --output .env.example \
  https://raw.githubusercontent.com/AzeevAN/mcp-1c/v3.0.0/.env.example
```
...
MCP1C_PORT=5001
MCP1C_IMAGE=ghcr.io/azeevan/mcp-1c:3.0.0
MCP1C_DASHBOARD=on
MCP1C_ACCESS=local
...
| `MCP1C_IMAGE` | готовый OCI-образ или точный digest | `ghcr.io/azeevan/mcp-1c:3.0.0` |
...
`parser_version=10`. Само обновление кода не запускает переразбор источников.
...
notation `{namespace}name`. Отсутствующие `elementFormQualified`,
