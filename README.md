...
| Тесты | `.venv/bin/python -m pytest`, 2115 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2115 тестов (прогон 2026-09-05)
```
...
наблюдателя, Git и инкрементального parser-а. Открытие страницы только
перечисляет корни и привязки, не хеширует их содержимое. Проверка `incoming`
