...
def _сведения_о_выгрузке(path: Path) -> tuple[bool, str]:
    ...
    Правило распознавания — положительное с обеих сторон, а не «не нашли
    расширение, значит конфигурация»:

    - **Расширение**, если сошлись все четыре условия сразу: `ObjectBelonging`
      и `ConfigurationExtensionPurpose` есть, `NamePrefix` непустой, и
      `CompatibilityMode` отсутствует. Проверено на живой паре «конфигурация
      + её расширение», CHANGELOG → «Найдено».
    - **Конфигурация**, если и только если `CompatibilityMode` непустой.
      Замер ревью на обеих реальных конфигурациях: `Version8_3_21` у
      иерархической выгрузки, `DontUse` у плоской (8.3.5). `NamePrefix` у
      конфигурации ведёт себя по-разному: иерархическая несёт тег пустым
      (`<NamePrefix/>`), а у плоской 8.3.5 тега нет вовсе — полагаться на
      его наличие или отсутствие нельзя, надёжен только `CompatibilityMode`.
      `ConfigurationExtensionCompatibilityMode` — ложный признак, он стоит у
      обеих сторон, отличить по нему нельзя.
    - **Иначе — отказ (`RegistryError`), без исключений.** Архив без
      `Configuration.xml` вовсе (ни в корне, ни в обёртке), нечитаемый
      (`BadZipFile`/`OSError` — например, побитый CRC именно у
      `Configuration.xml`), неразбираемый как XML, без узнаваемых
      `Properties`, или с частичным набором признаков (виден один из
      сильных сигналов расширения, но не все условия сошлись, а
      `CompatibilityMode` при этом пуст) — во всех случаях причина отказа
      называется в тексте.
    ...
    и снесёт уже разобранный код конфигурации. Молчаливый `(False, "")` в
    неоднозначных случаях был неверен по той же причине — он означал «нет
    сомнений, это конфигурация» именно там, где сомнение и было. Поэтому
    нечитаемый, пустой и наполовину похожий на расширение манифест попадают
    в отказ автоматически: решает только явный положительный признак одной
    из двух сторон, а не отсутствие признаков другой.
    """
    ...
    raise отказ(
        "Не нашли ни полного набора признаков расширения (ObjectBelonging, "
        "ConfigurationExtensionPurpose, непустой NamePrefix, отсутствие "
        "CompatibilityMode — сейчас: "
        f"ObjectBelonging={значение('ObjectBelonging') or '—'}, "
        f"ConfigurationExtensionPurpose={значение('ConfigurationExtensionPurpose') or '—'}, "
        f"NamePrefix={значение('NamePrefix') or '—'}, "
        f"CompatibilityMode={значение('CompatibilityMode') or '—'}), "
        "ни непустого CompatibilityMode конфигурации — набор признаков "
        "неполный."
    )
