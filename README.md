...
| Тесты | `.venv/bin/python -m pytest`, 2594 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2594 теста (прогон 2026-09-13)
```
...
списки выбора. Для групп доступны ориентация, представление и видимость
...
конфликты возможностей должны блокироваться точечно. События, зависящие от
типа основного реквизита, пока остаются за границей контракта.
...
12 970 байт и приблизительно **3 010 токенов** для четырёх схем Forms при
