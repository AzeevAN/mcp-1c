...
CONFIG_PARAM = Annotated[
    ...
    Field(
        description=(
            ...
            "(например «РозницаДляКазахстана»). Обязателен, если загружено "
...
3. `get_object` — состав объекта. Уровень `brief` для беглого взгляда,
   `fields` для написания кода, `full` когда нужны и связи.
4. `get_related` — что ещё задевает задача: движения документа, кто ссылается
   на справочник.
5. `search_syntax` / `get_syntax` — методы и свойства платформы, а также язык
...
Не пропускайте шаг 3. Поиск отдаёт только имена и счётчики; всё, от чего
...
Сервер описывает структуру конфигурации, а не её данные, и ничего не знает
про код конкретного проекта — его читайте из файлов.
""".strip()
