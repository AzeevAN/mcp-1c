...
| Тесты | `.venv/bin/python -m pytest`, 2193 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2193 теста (прогон 2026-09-08)
```
...
`parser_version=8`. Само обновление кода не запускает переразбор источников.
...
| `get_object` | поля, их доказанное происхождение, HTTP-endpoint, таблицы запроса, связи и кодовые сведения объекта |
