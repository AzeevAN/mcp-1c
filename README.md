...
| Тесты | `.venv/bin/python -m pytest`, 2626 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2626 тестов (прогон 2026-09-13)
```
...
путь вида `Объект.Поле`. Автономный compiler не проверяет существование
...
зависящие от типа основного реквизита, составные типы и `DynamicList` пока
...
13 912 байт и приблизительно **3 188 токенов** для четырёх схем Forms при
