...
| Тесты | `.venv/bin/python -m pytest`, 2457 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2457 тестов (прогон 2026-09-12)
```
...
арности остаётся предупреждением. Успешная статическая операция не считается
...
на macOS/Python 3.12 (10 прогонов, 2026-09-12): медиана 2,837 мс на batch,
диапазон 2,709–3,376 мс. Запрос занимал 925 байт, compile-ответ — 8 884,
check — 4 194, decompile — 3 458, самый большой раздел правил — 2 999 байт.
