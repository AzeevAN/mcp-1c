...
| Тесты | `.venv/bin/python -m pytest`, 2688 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2688 тестов (прогон 2026-09-13)
```
...
либо подменю — и всегда получает `ExtendedTooltip`; доступны необязательные
`title`, `horizontal_location`, `horizontal_stretch` и `vertical_stretch`.
Подменю `popup` требует локализованный `title`, одну или более прямых кнопок и
также получает `ExtendedTooltip`. Собственных событий у панели и подменю нет.
`ButtonGroup`, картинки, специальные представления, автоматический
`CommandSource` и контейнеры без явных кнопок пока сохраняются только в
inventory.
...
18 565 байт и приблизительно **4 366 токенов** для четырёх схем Forms при
