...
| Тесты | `.venv/bin/python -m pytest`, 2084 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2084 теста (прогон 2026-09-05)
```
...
`parser_version=6`. Само обновление кода не запускает переразбор источников.
