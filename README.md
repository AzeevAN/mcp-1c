...
| Тесты | `.venv/bin/python -m pytest`, 2770 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2770 тестов (прогон 2026-09-14)
```
...
| `get_object` | поля, их доказанное происхождение, постраничные HTTP-endpoint с продолжением по `cursor`, XDTO-пакеты и типы, таблицы запроса, связи и кодовые сведения объекта |
