...
| Тесты | `.venv/bin/python -m pytest`, 1938 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1938 тестов (прогон 2026-09-03)
```
...
вида объекта идут первыми, технические идентификаторы — вторыми. Обычный режим
группирует предоставленные права карточками объектов и не рисует строки
`false`; дочерние цели и технический аудит открываются отдельно. Каталог,
объекты, детали и кандидаты читаются серверными страницами, RLS раскрывается
только отдельной кнопкой. Состояния пустого Registry, `missing` и `error`
объясняются, а операций назначения или редактирования ролей на странице нет.
