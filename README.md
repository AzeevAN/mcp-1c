...
| Тесты | `.venv/bin/python -m pytest`, 2431 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2431 тест (прогон 2026-09-12)
```
...
не пишут и не обращаются к Registry или 1С. Успешная статическая операция не
считается нативным импортом или визуальной приёмкой: эти уровни результата
