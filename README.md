...
| Тесты | `.venv/bin/python -m pytest`, 1967 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1967 тестов (прогон 2026-09-04)
```
...
после обновления страницы или рестарта сервера; кнопка называет конфигурацию и
исходный ZIP, а не только внутренний `candidate_id`. «Сохранить preview и
вернуться» оставляет его для продолжения, «Отменить и удалить preview» явно
снимает job и её рабочие данные. До отправки `confirm` ни одно из этих действий
ничего не публикует. После отправки серверная атомарная публикация уже
...
удаляются. Удаление отдельного расширения по-прежнему затрагивает только его
поколение. Операция требует ввода точного идентификатора; `incoming/` и
read-only исходники она не изменяет, поэтому оставшийся там ZIP снова виден
как кандидат создания. Повтор того же административного удаления по точному
имени также завершает очистку durable jobs, если Registry уже был снят прежней
версией сервера.
