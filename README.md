...
| Тесты | `.venv/bin/python -m pytest`, 2658 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2658 тестов (прогон 2026-09-13)
```
...
CRLF. Поддержаны рекурсивные группы, страницы, поля ввода, флажки, кнопки,
...
заголовка; для групп, страниц, таблиц и полей ввода — явное растяжение. Агент
...
`CustomizeForm`; для `command_kind=item_standard` с обязательным
`command_owner` таблицы — `Add`, `Delete`, `MoveUp` и `MoveDown`. Эти ссылки
не требуют записи в `commands` и не создают BSL-каркас `Action`. Объектные
команды вроде записи и проведения остаются за границей текущего контракта.
...
конфликты возможностей должны блокироваться точечно. События и команды,
зависящие от типа основного реквизита, пока остаются за границей контракта.
...
15 180 байт и приблизительно **3 503 токена** для четырёх схем Forms при
