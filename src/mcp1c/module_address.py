...
выгрузках («Розница для Казахстана» и обезличенной отраслевой) таблица сейчас разбирает
...
после того, как такой пример нашёлся на обезличенной отраслевой выгрузке.
...
# обеих живых выгрузках. У Bot официальный объект имеет один модуль, а его
# tree-путь Ext/Module.bsl зафиксирован по принятому фактическому handoff;
# в текущем локальном корпусе ботов нет. Суффикс для этих видов не несёт
# различающего смысла, поэтому в адресе его нет. WebServices и HTTPServices
# сюда не входят: их Module.bsl уже имеет пустой суффикс в _МОДУЛИ.
_ОДИН_МОДУЛЬ = {
    "Bots": "Module.bsl",
