...
| Тесты | `.venv/bin/python -m pytest`, 2671 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2671 тест (прогон 2026-09-13)
```
...
надписи — `Click` и `URLProcessing`. Compiler
...
15 904 байта и приблизительно **3 679 токенов** для четырёх схем Forms при
