...
| Тесты | `.venv/bin/python -m pytest`, 2710 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2710 тестов (прогон 2026-09-13)
```
...
Панель команд `command_bar` требует один или более прямых элементов — кнопок,
подменю либо групп кнопок — и всегда получает `ExtendedTooltip`; доступны
необязательные `title`, `horizontal_location`, `horizontal_stretch` и
`vertical_stretch`. Подменю `popup` требует локализованный `title`, одну или
более кнопок либо групп кнопок и также получает `ExtendedTooltip`. Группа
кнопок `button_group` требует одну или более прямых кнопок, допускает
необязательный `title` и представление `usual` либо `compact`. Собственных
событий у этих контейнеров нет. Картинки, специальные представления,
автоматический `CommandSource` и более глубокая вложенность пока сохраняются
только в inventory.
...
19 401 байт и приблизительно **4 559 токенов** для четырёх схем Forms при
