...
| Тесты | `.venv/bin/python -m pytest`, 2292 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2292 теста (прогон 2026-09-12)
```
...
Неизвестный узел сохраняется в semantic hash и выдаёт диагностику. Flat-имя и
