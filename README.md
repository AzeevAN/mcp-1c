...
| Тесты | `.venv/bin/python -m pytest`, 2241 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2241 тест (прогон 2026-09-12)
```
...
restart; для расширения без доказанной привязки к конкретному поколению базы
ответ остаётся `unknown`.
