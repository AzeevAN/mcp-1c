...
`search_syntax`/`get_syntax` уже находят элементы обоих доменов — поиск и
точное совпадение это умеют с задачи 3 (`registry.py`, `LoadedSyntax`). Но
...
def test_конфигурация_и_язык_запросов_без_справки_платформы_не_падают(tmp_path):
    ...
    постановке обязан работать без справки платформы. Найдено при подготовке
    задачи 4, не описано в брифе явно."""
    incoming = tmp_path / "incoming"
    registry = Registry(tmp_path / "data")
    registry.add_syntax(query_hbk_stub(incoming))
    config = build_configuration()
...
def test_оговорка_не_путает_отсутствие_платформы_с_неопределённой_версией(tmp_path):
    ...
    Отмечено исполнителем задачи 3 как находка, требующая исправления здесь."""
    incoming = tmp_path / "incoming"
    registry = Registry(tmp_path / "data")
    registry.add_syntax(query_hbk_stub(incoming))
    config = build_configuration()
...
def test_query_table_версионируется_как_платформа(tmp_path):
    """Найдено ревью: `_is_query_kind` по префиксу `query_` ловил и
...
def test_одноимённые_из_разных_доменов_разводятся_видом(tmp_path):
    ...
    # местами — ревью задачи 4 отметило это как слабое место теста.
