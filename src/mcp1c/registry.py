...
def _единственный_верхний_каталог(zf: zipfile.ZipFile) -> str | None:
    ...
    `None`, если однозначного каталога нет: в корне лежит хотя бы один файл,
    или каталогов верхнего уровня несколько. Это не наш случай — гадать,
    какой из них «тот самый», рискованнее отказа.
    """
...
def _сведения_о_выгрузке(path: Path) -> tuple[bool, str]:
    ...
    Цена ошибки здесь несимметрична: принять расширение за конфигурацию —
    значит вызывающий пойдёт в ветку модулей и снесёт уже разобранный код
    конфигурации. Поэтому там, где по правилу выше решить нельзя однозначно,
    функция отказывает явно (`RegistryError`), а не молча возвращает
    `(False, "")` — тихий переход в разрушительную ветку здесь не годится:

    - `Configuration.xml` не нашёлся вовсе (ни в корне, ни в единственном
      каталоге верхнего уровня) — определить вид нечем;
    - виден хотя бы один сильный признак расширения (`ObjectBelonging` или
      `ConfigurationExtensionPurpose`), но остальные условия не сошлись —
      похоже на расширение, собранное не так, как ожидалось, а не на
      конфигурацию.

    Мягкий возврат `(False, "")` остаётся там, где `Configuration.xml`
    найден, но не несёт узнаваемой структуры MDClasses вовсе (в том числе
    синтетические архивы тестов модулей, несущие `<x/>` — без `Properties`
    решать нечего, и это не признак расширения ни с одной стороны) или явно
    описывает конфигурацию (нет ни одного из двух сильных признаков). Так же
    обрабатывается настоящая выгрузка конфигурации — вызывающий продолжает
    её как модули.
    """
    with zipfile.ZipFile(path) as zf:
        ...
        if сведения is None:
            raise RegistryError(
                f"{path.name}: в архиве не нашлось Configuration.xml ни в "
                "корне, ни в единственном каталоге верхнего уровня — "
                "определить, конфигурация это или расширение, нечем."
            )
        if сведения.file_size > _MAX_CONFIGURATION_XML_SIZE:
            raise RegistryError(
                f"{path.name}: Configuration.xml весит "
                f"{сведения.file_size / 1024 / 1024:.1f} МБ по заявленному "
                "размеру в архиве — это на порядок больше боевой "
                "конфигурации (847 КБ), похоже на повреждённый или "
                "поддельный архив."
            )
    ...
    сильный_признак = bool(значение("ObjectBelonging")) or bool(
    ...
    if сильный_признак:
        # Хотя бы один сильный признак расширения есть, но не все условия
        # сошлись — это не конфигурация (у неё нет ни одного из этих двух
        # тегов вовсе). Молчаливый переход в ветку модулей здесь опаснее
        # отказа: см. докстроку выше.
        raise RegistryError(
            f"{path.name}: похоже на выгрузку расширения (есть "
            "ObjectBelonging или ConfigurationExtensionPurpose), но набор "
            "признаков неполный — CompatibilityMode="
            f"{значение('CompatibilityMode') or '—'}, NamePrefix="
            f"{значение('NamePrefix') or '—'}. У настоящей конфигурации "
            "нет ни одного из этих двух тегов, поэтому считать архив "
            "конфигурацией здесь тоже неверно — определить вид выгрузки "
            "нечем, отказ."
        )
    return False, ""
...
class Registry:
    ...
    def _extract_code(
        ...
        """Хеш и распаковка кода в `корень`: снос старого перед записью,
        отказ на пустом результате. Общее между модулями конфигурации и
        расширением — разное у них только каталог (значит, и функция сноса,
        переданная `drop`) и то, как затем назвать источник.
        """
        from . import intake
        ...
        drop(корень)
        файлов, _байт = intake.extract(архив, корень)
        if not файлов:
