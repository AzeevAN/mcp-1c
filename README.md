...
| Тесты | `.venv/bin/python -m pytest`, 2116 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2116 тестов (прогон 2026-09-06)
```
...
Это ручной полный разбор с существующим semantic diff/no-op, без фонового
наблюдателя, Git и инкрементального parser-а. Кнопки каталога получают состояние
