...
Сервер — один Python-процесс без внешней базы данных. При старте он
восстанавливает `Registry`, поднимает поисковые индексы и обслуживает MCP,
health, административный API и выбранный дашборд.
...
PYTHONPATH=src .venv/bin/python -m mcp1c.bench \
  ...
  --auto --sets query-language,metadata,modules-procedures \
  --check-notes
```
