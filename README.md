...
| Тесты | `.venv/bin/python -m pytest`, 2520 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2520 тестов (прогон 2026-09-12)
```
...
строгое подмножество спецификации управляемой формы и детерминированно
возвращает тексты `Form.xml` версии `2.16` и `Form/Module.bsl` с UTF-8 BOM и
CRLF. `check_managed_form(form_xml, form_name, module_bsl?)` статически проверяет
...
6 233 байта и приблизительно **1 422 токена** для четырёх схем Forms при
