...
| Тесты | `.venv/bin/python -m pytest`, 2676 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2676 тестов (прогон 2026-09-13)
```
...
CRLF. Поддержаны рекурсивные группы, страницы, поля ввода, флажки, статические
и связанные с данными надписи, кнопки,
...
`OnChange`, `Click` и `URLProcessing`. Compiler
...
16 628 байт и приблизительно **3 868 токенов** для четырёх схем Forms при
