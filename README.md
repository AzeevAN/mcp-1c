...
| Код и расширения | процедуры, тела, формы, места вызовов; отдельный сеансовый снимок активности |
...
| Тесты | `.venv/bin/python -m pytest`, 1793 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1793 теста (прогон 2026-08-29)
```
...
Без отдельного снимка `list_extensions` возвращает `unknown`. Позиция в нём —
порядок элементов, возвращённый API платформы, а не обещание порядка исполнения
модулей.
