...
**Русская морфология.** «накладная» и «накладной» — одно слово. Snowball
сводит словоформы к общей основе; прежнее усечение до шести символов не
справлялось с короткими формами вроде «чек», «чека» и «чеков».
...
DEFAULT_FIELD_WEIGHTS = {
    ...
    "part": 4.0,
    "kind": 1.2,
    # Поисковые ключи справки (`search_keys.py`) — формулировки задачи,
    # которыми спрашивают платформенный метод с непохожим именем.
    "keys": 2.0,
    "description": 0.6,
}
...
OWNER_KIND_BOOST = {
    ...
    "Отчет": 0.3,
    "Обработка": 0.3,
    # Подсистема полезна как навигационный слой по явному kind-фильтру и
    # прямому полному имени. Без фильтра её точный синоним не должен вытеснять
    # прикладную обработку или модуль с теми же словами.
    "Подсистема": 0.05,
}
...
# Потолок описания в индексе. Имя, родитель и краткое начало описания дают
# достаточно сигнала, а длинный текст только раздувает постинги.
DESCRIPTION_CAP = 400
    ...
    def search(
        ...
        *,
        ...
        if predicate is not None:
            # `search_procedures` передаёт предикат для раздельного отбора
            # категорий до общего top-N: иначе нужный scope может потеряться
            # за более сильными кандидатами. Условие смотрит в payload и
            # задаётся потребителем, поэтому готовой векторной маски для него
            # нет — здесь остаётся обычный цикл по найденным документам.
            keep = np.fromiter(
        ...
        for position in order:
            ...
            if is_alias[position]:
                ...
                # До этой точки дошёл только найденный по токенам документ:
                # псевдоним, точная строка и полное покрытие уже разобраны выше.
                # Причина вычисляется после сортировки и не участвует в оценке.
                reason = "совпала часть слов запроса"
            hits.append(
                Hit(
        ...
        docs.append(
            Doc(
                ...
                fields={
                    ...
                    "description": item.description[:DESCRIPTION_CAP],
                },
            )
        )
