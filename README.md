...
| Тесты | `.venv/bin/python -m pytest`, 2756 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2756 тестов (прогон 2026-09-13)
```
...
- получает готовый образ `ghcr.io/azeevan/mcp-1c:3.1.0` без локальной сборки;
...
1. Обычный пользователь скачивает готовый
   `ghcr.io/azeevan/mcp-1c:3.1.0` и запускает один `compose.yaml` по инструкции
   ниже.
...
curl --fail --show-error --location --output compose.yaml \
  https://raw.githubusercontent.com/AzeevAN/mcp-1c/v3.1.0/compose.yaml
curl --fail --show-error --location --output .env.example \
  https://raw.githubusercontent.com/AzeevAN/mcp-1c/v3.1.0/.env.example
```
...
MCP1C_PORT=5001
MCP1C_IMAGE=ghcr.io/azeevan/mcp-1c:3.1.0
MCP1C_DASHBOARD=on
MCP1C_ACCESS=local
...
| `MCP1C_IMAGE` | готовый OCI-образ или точный digest | `ghcr.io/azeevan/mcp-1c:3.1.0` |
...
`parser_version=11`. Само обновление кода не запускает переразбор источников.
...
`MCP1C_CAPABILITIES=off|diagnostics` как bootstrap при отсутствии
...
| `diagnostics_status` | условно: статический статус демонстрационного capability-модуля без чтения данных |
...
Имя `diagnostics` добавляет ровно один read-only инструмент
`diagnostics_status`. Имя `forms` добавляет ровно четыре инструмента в
стабильном порядке: `get_managed_form_rules`, `compile_managed_form`,
`decompile_managed_form`, `check_managed_form`. `diagnostics` не читает
Registry; Forms получает только read-only контекст через штатный интерфейс
...
| `diagnostics` | 512,507 (498,626–538,156) | 88,711 (87,875–89,391) |
