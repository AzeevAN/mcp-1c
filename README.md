...
| Тесты | `.venv/bin/python -m pytest`, 2647 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2647 тестов (прогон 2026-09-13)
```
...
зависящие от типа основного реквизита, и `DynamicList` пока
остаются за границей контракта.
...
14 792 байта и приблизительно **3 411 токенов** для четырёх схем Forms при
