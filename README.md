...
| Тесты | `.venv/bin/python -m pytest`, 2527 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2527 тестов (прогон 2026-09-12)
```
...
CRLF. Поддержаны рекурсивные группы, страницы, поля, кнопки, таблицы значений,
строки, boolean, числа, даты, multiline/read-only и строковые списки выбора.
Агент выбирает смысловую компоновку по теме правил `layout`; compiler сохраняет
её порядок и ничего автоматически не переставляет.
...
10 182 байта и приблизительно **2 439 токенов** для четырёх схем Forms при
