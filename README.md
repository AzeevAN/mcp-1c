...
| Тесты | `.venv/bin/python -m pytest`, 1960 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1960 тестов (прогон 2026-09-04)
```
...
отклоняется до смены active generation.
...
сеансовый снимок и все привязанные расширения. Удаление отдельного расширения
по-прежнему затрагивает только его поколение. Операция требует ввода точного
идентификатора; `incoming/` и исходные архивы она не изменяет.
