...
| Тесты | `.venv/bin/python -m pytest`, 2762 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2762 тест (прогон 2026-09-14)
```
...
`selection_version=9` и `parser_version=12`. Старые поколения не содержат
descriptor-слой критериев отбора; само обновление кода не запускает переразбор
источников.
