...
5. `get_object` — состав объекта. Уровень `brief` для беглого взгляда,
   `fields` для написания кода, `full` когда нужны и связи.
6. `get_related` — что ещё задевает задача: движения документа, кто ссылается
   на справочник.
7. `search_syntax` / `get_syntax` — методы и свойства платформы, а также язык
...
Не пропускайте шаг 5. Поиск объектов отдаёт только имена и счётчики; всё, от чего
