...
| Тесты | `.venv/bin/python -m pytest`, 2444 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2444 теста (прогон 2026-09-12)
```
...
Сейчас разрешено единственное демонстрационное имя `diagnostics`; оно добавляет
ровно один read-only инструмент `diagnostics_status`, который не читает
Registry, `data/` или локальные проекты и ничего не записывает.

В исходниках также развивается пакет `forms`, но он ещё не зарегистрирован как
capability и не появляется в `tools/list`. Его внутренний compiler принимает
...
CRLF. Парный decompiler восстанавливает каноническую спецификацию этого
подмножества; более широкие формы и форматы `2.19`/`2.20` читает только как
