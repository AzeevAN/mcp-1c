...
| Тесты | `.venv/bin/python -m pytest`, 2618 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2618 тестов (прогон 2026-09-13)
```
...
`decompile_managed_form`, `check_managed_form`. Оба модуля не читают Registry,
`data/` или локальные проекты и ничего не записывают.
...
единственный объект `specification` —
...
путь вида `Объект.Поле`. На этом этапе существование объекта и поля ещё не
проверяется через Registry и остаётся `not_checked`; имя конкретной
конфигурации в спецификацию и XML не попадает. Для групп доступны ориентация,
...
`check_managed_form(form_xml, form_name, module_bsl?)` статически проверяет
результат. `decompile_managed_form` с теми же параметрами восстанавливает
...
Вход `Form.xml` ограничен 2 МиБ, DTD и ENTITY запрещены. Оба компонента ничего
не пишут и не обращаются к Registry или 1С. Внутренний checker отдельно
...
ошибку чтения либо использовать асинхронный API платформы. Успешная статическая
операция не считается проверкой Registry, нативным импортом или визуальной
приёмкой: эти уровни остаются `not_checked`.
...
13 271 байт и приблизительно **3 080 токенов** для четырёх схем Forms при
