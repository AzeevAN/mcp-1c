...
def test_скомпилированный_модуль_виден_но_процедуры_не_объявляются(
    ...
    assert "модулей 1" in список and "недоступным исходным текстом 1" in список
    assert "ОбщийМодуль.Закрытый" in объект and "исходный текст недоступен" in объект
    assert "исходный текст недоступен" in модуль
    assert "исходный текст недоступен" in процедура
    assert "исходный текст недоступен" in поиск
    assert "исходный текст недоступен" in вызовы
...
def test_скомпилированный_модуль_расширения_остаётся_отдельным_корпусом(
    ...
    assert "исходный текст недоступен" in ответ
...
def test_скомпилированная_часть_расширения_делает_сводку_и_перекрытия_частичными(
    ...
    assert "исходный текст недоступен" in связи
    assert "пустой результат не доказывает" in связи
    assert связи.index("исходный текст недоступен") < связи.index("## Ссылается")
...
def test_типовая_фраза_без_scope_не_выбирает_случайный_адрес(
    корень_кода, реестр_из_кода
):
    реестр = реестр_из_кода(корень_кода)

    ответ = tools.search_procedures(
        реестр, "что выполняется при записи объекта", config="Пример"
    )

    assert "Распознано типовое событие `ПриЗаписи`" in ответ
    assert "Найдено реализаций: 1" in ответ
    assert "scope" in ответ
    assert "Документ.Пример.МодульОбъекта::ПриЗаписи" not in ответ


def test_типовая_фраза_с_точным_модулем_возвращает_реализацию(
    корень_кода, реестр_из_кода
):
    реестр = реестр_из_кода(корень_кода)
    ...
    ответ = tools.search_procedures(
        реестр,
        "что выполняется при записи объекта",
        config="Пример",
        scope="Документ.Пример.МодульОбъекта",
    )

    assert "Распознано типовое событие `ПриЗаписи`" in ответ
    assert "Документ.Пример.МодульОбъекта::ПриЗаписи" in ответ
    assert "Сигнатура: `Процедура ПриЗаписи(Отказ)`" in ответ


def test_типовая_фраза_с_неоднозначным_scope_просит_точный_модуль(
    корень_кода, реестр_из_кода
):
    менеджер = корень_кода / "Documents" / "Пример" / "Ext" / "ManagerModule.bsl"
    менеджер.write_text(
        "Процедура ПриЗаписи()\nКонецПроцедуры\n", encoding="utf-8"
    )
    реестр = реестр_из_кода(корень_кода)
    ...
    assert "Распознано типовое событие `ПриЗаписи`" in ответ
    assert "В области найдено реализаций: 2" in ответ
    assert "точный адрес модуля или формы" in ответ
    assert "::ПриЗаписи" not in ответ


def test_контрольная_фраза_остаётся_в_обычном_поиске(
    корень_кода, реестр_из_кода
):
    общий = корень_кода / "CommonModules" / "Остатки" / "Ext"
    ...
    ответ = tools.search_procedures(
        реестр, "проверить остатки на складе", config="Пример"
    )

    assert "ОбщийМодуль.Остатки::ПроверитьОстатки" in ответ
    assert "Распознано типовое событие" not in ответ
...
# --- Блоки кода в существующих ответах ---------------------------------
...
def _конфигурация_для_блоков() -> object:
    ...
    документ = MetadataObject(
        ...
        name="Пример",
        synonym="Пример",
        # `Ссылка` — зарезервированный платформенный реквизит документа.
        # Для проверки связи нужен обычный реквизит с допустимым именем.
        attributes=[Field(name="СвязанныйОбъект")],
    )
...
# --- Согласованность списка и классификация форм ------------------------
