...
| Тесты | `.venv/bin/python -m pytest`, 2749 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2749 тестов (прогон 2026-09-13)
```
...
возвращает тексты `Form.xml` версии `2.16` и `Form/Module.bsl` с UTF-8 BOM и
CRLF. Поддержаны рекурсивные группы, страницы, поля ввода, флажки,
...
управляемую форму формата `2.16`. Необязательное поле `platform_version`
...
22 419 байт и приблизительно **5 363 токена** для четырёх схем Forms при
