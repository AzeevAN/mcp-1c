...
только редкие каталоги вроде `Sequences`, но и обычные `HTTPServices`,
`WebServices`, `Constants` — так что «неизвестный» не значит «редкий».
Решение по-прежнему то же — честный отказ (`ValueError` с именем каталога),
а не подстановка похожего вида: собранный из чужого вида объекта метаданных
...
для проверки — то самое молчаливое искажение: `Sequences` в корпусе
«Розницы» не встретился ни разу и в таблице по-прежнему отсутствует.
