...
| Тесты | `.venv/bin/python -m pytest`, 2692 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2692 теста (прогон 2026-09-13)
```
...
запрашивает только нужные тематические разделы. `compile_managed_form` принимает
...
19 111 байт и приблизительно **4 504 токена** для четырёх схем Forms при
