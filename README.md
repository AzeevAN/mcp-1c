...
| Тесты | `.venv/bin/python -m pytest`, 2587 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2587 тестов (прогон 2026-09-13)
```
...
профиль для compiler, decompiler и checker. Сейчас компилируется интервал
8.3.23–8.3.27; 8.3.23, 8.3.26 и 8.3.27 подтверждены справкой, а 8.3.24–8.3.25
покрыты правилом совместимости интервала. Профиль 8.3.5 хранит известные
сигнатуры отдельно, но остаётся справочным до доказательства совместимого
`Form.xml`. События, зависящие от типа основного реквизита, также остаются за
границей контракта.
...
12 629 байт и приблизительно **2 973 токена** для четырёх схем Forms при
