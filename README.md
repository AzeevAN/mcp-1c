...
| Тесты | `.venv/bin/python -m pytest`, 2214 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2214 тестов (прогон 2026-09-08)
```
...
`parser_version=9`. Само обновление кода не запускает переразбор источников.
