...
4. `get_object` — состав объекта. Уровень `brief` для беглого взгляда,
   `fields` для написания кода, `full` когда нужны и связи.
5. `get_related` — что ещё задевает задача: движения документа, кто ссылается
   на справочник.
6. `search_syntax` / `get_syntax` — методы и свойства платформы, а также язык
   запросов: `ВЫБРАТЬ`, `ЛЕВОЕ СОЕДИНЕНИЕ`, `ИТОГИ ПО`, `ВЫРАЗИТЬ`, функции
   вроде `КОНЕЦПЕРИОДА`. Выдача по платформе отфильтрована по версии
   конфигурации: то, чего в ней нет, не покажется. У языка запросов версии
   нет — этот фильтр его не касается. Обращайте внимание на поле
   «Доступность» — вызов серверного метода из клиентского контекста не
   скомпилируется.

Не пропускайте шаг 4. Поиск отдаёт только имена и счётчики; всё, от чего
...
по значениям документов, справочников и регистров.
""".strip()
...
def build_server(registry: Registry, name: str = "mcp1c") -> MCPServer:
    ...
    @server.tool(
        description=(
            ...
            "процедуры; поиск по словам — только экспортные. `scope` задаёт "
            "явный приоритет модулей объекта или одного точного модуля, но не "
            "фильтрует остальную выдачу и не угадывается из query. Сигнатуры "
    ...
    def search_procedures(
        query: Annotated[str, Field(
            description=(
                ...
                "комментария-шапки (`проверить остатки`). Объект поиска из "
                "этого текста не угадывается — для него есть `scope`."
            )
        )],
        config: CONFIG_PARAM = None,
        ...
        scope: Annotated[str | None, Field(
            description=(
                "Необязательный явный scope: объект (`Документ.ЧекККМ`) "
                "поднимает все его модули, точный адрес "
                "(`ОбщийМодуль.ОбщегоНазначения`) — один модуль. Остальные "
                "результаты остаются ниже; из query scope не выводится."
            )
        )] = None,
    ...
    @server.tool(
        description=(
            ...
            "языка запросов (функцию, ключевое слово, статью). Выдача по "
            "платформе отфильтрована по версии конфигурации; язык запросов "
            "версий не имеет и фильтром не затрагивается. "
