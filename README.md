...
| Тесты | `.venv/bin/python -m pytest`, 2450 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2450 тестов (прогон 2026-09-12)
```
...
6 041 байт и приблизительно **1 385 токенов** для четырёх схем Forms при
...
|---|---:|---:|
| `off` | 523,363 (510,803–549,254) | 88,734 (88,094–89,641) |
| `diagnostics` | 523,237 (505,144–528,623) | 88,734 (88,172–89,234) |

Разница меньше разброса запусков и не доказывает ускорение режима `on`; в этом
синтетическом срезе измеримого штрафа старта или RSS не обнаружено. Повторить
тем же скриптом:
