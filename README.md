...
| Тесты | `.venv/bin/python -m pytest`, 2242 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 2242 теста (прогон 2026-09-12)
```
...
`data/`: прежние поколения с отдельными member-файлами остаются читаемыми.
