...
| Тесты | `.venv/bin/python -m pytest`, 1858 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1858 тестов (прогон 2026-09-02)
```
...
| ZIP выгрузки конфигурации в файлы | **Конфигуратор → Конфигурация → Выгрузить конфигурацию в файлы… → Архив** | `Configuration.xml`, структура, модули, формы и роли | двухфазный intake API из browser-upload, `data/incoming/` или `MCP1C_CONFIG_SOURCE`; прежняя кнопка «Разобрать» остаётся для code-only пути |
...
Прежняя команда `POST /api/v1/sources/incoming/parse` не создаёт конфигурацию
в Registry: она добавляет модули и формы к уже загруженной структуре, а также
строит компактный каталог
