    ...
    if reference.provider is not None:
        ...
        @server.tool(
            description=(
                ...
                "Возвращает короткие карточки и причину совпадения; полный "
                "текст читается через `get_reference` по точному `id`."
            )
        )
        ...
        def get_reference(
            ...
            platform: Annotated[str | None, Field(
                description=(
                    ...
                    "отображения совместимости карточки, а не для выбора текста."
                )
            )] = None,
        ) -> str:
            try:
