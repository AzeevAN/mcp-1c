...
| Тесты | `.venv/bin/python -m pytest`, 1947 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1947 тестов (прогон 2026-09-03)
```
...
после обновления страницы или рестарта сервера. До отправки `confirm` выход из
preview ничего не публикует. После отправки серверная атомарная публикация уже
запущена: диалог блокирует закрытие и повторный intake до ответа, а обновление
или закрытие страницы не является командой rollback; durable результат
остаётся в состоянии job.
...
`sources`. Корзина карточки «Основная конфигурация» снимает только слои `code`
и `forms`, их индекс и журнал: `base_structure`, `extended_structure` и роли
остаются в новом атомарном поколении. Отдельная кнопка «Удалить» в шапке
конфигурации каскадно снимает саму структуру, основной корпус и все привязанные
расширения. Оба действия требуют ввода точного идентификатора; `incoming/` и
исходные архивы они не изменяют.
