...
| Тесты | `.venv/bin/python -m pytest`, 1716 |

Воспроизводимый прогон:

```bash
.venv/bin/pip install --require-hashes -r requirements-dev-lock.txt
.venv/bin/python -m pytest          # 1716 тестов (прогон 2026-08-30)
```
...
управлением оператора и отключает загрузку из дашборда. Значение `off`
